...
class SunoApi {
  ...
  private async getAuthToken() {
    ...
    const sessionResponse = await this.client.get(getSessionUrl, {
      headers: {
        Authorization: this.cookies.__client,
      },
    });
  ...
  private async getSessionToken() {
    const tokenResponse = await this.client.post(`${SunoApi.BASE_URL}/api/user/create_session_id/`, {
      session_properties: JSON.stringify({
  ...
  private async findSunoPromptBox(page: Page): Promise<Locator | null> {
    const candidates = [
      ...
      page.locator('[aria-label*="Song" i]').first(),
      page.locator('[aria-label*="Description" i]').first(),
      page.locator('[placeholder*="Song" i]').first(),
      page.locator('[placeholder*="Description" i]').first(),
    ];
  ...
  private async findSunoCreateButton(page: Page): Promise<Locator> {
    const candidates = [
      ...
      page.locator('button[aria-label="Create"]').locator('div.flex').first(),
    ...
    throw new Error('No se encontró el botón Create de Suno.');
  }
    ...
    return new Promise((resolve, reject) => {
      page.route('**/api/generate/v2/**', async (route: any) => {
        try {
          ...
          if (authorization) {
            this.currentToken = authorization.split('Bearer ').pop();
          }
  ...
  private async generateSongs(
    ...
    try {
      response = await this.client.post(`${SunoApi.BASE_URL}/api/generate/v2/`, payload, {
        timeout: 15000,
      });
    } catch (error: any) {
      ...
      const detail = error?.response?.data
        ...
        : error?.message || 'Error generando en Suno.';

      throw new Error(status ? `Suno respondió ${status}: ${detail}` : detail);
    }
    ...
    return response.data.clips.map((audio: any) => this.mapClip(audio));
  }
